# AGENTS.md

**Policy source of truth: [CLAUDE.md](CLAUDE.md)**

All execution rules, non-negotiables, PR requirements, coding conventions, and project standards are defined in `CLAUDE.md` and apply to every Codex session without exception. Do not duplicate or override policy text here.

This file contains only Codex-specific operational instructions — handoff mechanics and document ownership rules — that are not relevant to Claude Code and do not belong in the shared policy file.

## Starting an Issue

When told to "start on #NNN", first run `git fetch origin`, then read the issue body with `gh issue view NNN` for the full spec.

## One-Off Fixes

For small corrections not tied to a formal issue, no handoff file is needed. Branch from `origin/main` using `fix/`, `chore/`, or `docs/` prefix, make the change, run `pnpm ci:verify`, then create and auto-merge the PR as R1:

```bash
git fetch origin
git checkout -B fix/short-description origin/main
# make the fix
pnpm ci:verify
gh pr create --title "fix: ..." --body "..."
gh pr merge --squash --delete-branch
```

Do not append to `docs/project-status.md` for one-off fixes unless the change is meaningful enough to note.

## Ancillary Documents

- `docs/project-status.md` — append a one-line completion entry on your feature branch before opening the PR. This is the only tracking doc Codex touches.
- `docs/issues.md` — do not modify. Claude Code updates this on `main` after merge.

After fetching the issue:
1. Check the issue body for any **Dependencies** note. If a listed issue is not yet merged into `main`, stop and prompt the user — do not proceed.
2. Follow the spec exactly. Do not expand scope beyond the stated acceptance criteria.
